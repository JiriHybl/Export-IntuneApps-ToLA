...
├── AppInventory.pbit                         # Sample PowerBI template
├── README.md                                 # This file
...
2. **Share → Export to Power BI (as en M query)** — LA will download a `.txt` file containing the M query
3. **Power BI Desktop → Home → New data source → Empty query → Transform data → Advanced Editor** — create a blank query, open Advanced Editor and paste the M query from the `.txt` file
